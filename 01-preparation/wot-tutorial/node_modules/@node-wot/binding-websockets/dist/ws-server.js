            ...
            if (_this.ownServer) {
                ...
                _this.httpServer.once("listening", function () {
                    _this.httpServer.on("error", function (err) {
                        console.error("WebSocketServer on port " + _this.port + " failed: " + err.message);
                    });
                    resolve();
                });
    ...
    WebSocketServer.prototype.expose = function (thing) {
        ...
        if (this.getPort() !== -1) {
            ...
            var _loop_1 = function (eventName) {
                ...
                console.debug("WebSocketServer on port " + this_1.getPort() + " adding socketServer for '" + path + "'");
                this_1.socketServers[path] = new WebSocket.Server({ noServer: true });
                this_1.socketServers[path].on('connection', function (ws, req) {
                    console.log("WebSocketServer on port " + _this.getPort() + " received connection for '" + path + "' from " + core_1.Helpers.toUriLiteral(req.connection.remoteAddress) + ":" + req.connection.remotePort);
                    thing.subscribeEvent(eventName, function (data) {
                        ...
                        try {
                            ...
                            console.warn("HttpServer on port " + _this.getPort() + " cannot process data for Event '" + eventName + ": " + err.message + "'");
                    ...
                    ws.on("close", function () {
                        ...
                        console.log("WebSocketServer on port " + _this.getPort() + " closed connection for '" + path + "' from " + core_1.Helpers.toUriLiteral(req.connection.remoteAddress) + ":" + req.connection.remotePort);
                    });
                });
                for (var _i = 0, _a = core_1.Helpers.getAddresses(); _i < _a.length; _i++) {
                    ...
                    console.log("WebSocketServer on port " + this_1.getPort() + " assigns '" + href + "' to Event '" + eventName + "'");
                }
            };
            var this_1 = this;
