...
var CoapServer = (function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.server.listen(_this.port, _this.address, function () {
                _this.server.on('error', function (err) {
                    console.error("CoapServer for port " + _this.port + " failed: " + err.message);
                });
                resolve();
            });
        });
    ...
    CoapServer.prototype.handleRequest = function (req, res) {
        var _this = this;
        console.log("CoapServer on port " + this.getPort() + " received '" + req.method + "(" + req._packet.messageId + ") " + req.url + "' from " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
        res.on('finish', function () {
            console.log("CoapServer replied with '" + res.code + "' to " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
        });
        var requestUri = url.parse(req.url);
        var contentType = req.options['Content-Format'];
        if (req.method === "PUT" || req.method === "POST") {
            if (!contentType && req.payload) {
                console.warn("CoapServer on port " + this.getPort() + " received no Content-Format from " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
                        ...
                        if (req.method === "GET") {
                            if (req.headers['Observe'] === undefined) {
                                thing_1.readProperty(segments[3])
                                    ...
                                    .catch(function (err) {
                                    console.error("CoapServer on port " + _this.getPort() + " got internal error on read '" + requestUri.pathname + "': " + err.message);
                                    res.code = "5.00";
                                    res.end(err.message);
                                });
                            }
                            ...
                            if (!property_1.readOnly) {
                                ...
                                thing_1.writeProperty(segments[3], value)
                                    ...
                                    .catch(function (err) {
                                    console.error("CoapServer on port " + _this.getPort() + " got internal error on write '" + requestUri.pathname + "': " + err.message);
                                    res.code = "5.00";
                                    res.end(err.message);
                                });
                            }
                    ...
                    if (action_1) {
                        if (req.method === "POST") {
                            ...
                            thing_1.invokeAction(segments[3], input)
                                ...
                                .catch(function (err) {
                                console.error("CoapServer on port " + _this.getPort() + " got internal error on invoke '" + requestUri.pathname + "': " + err.message);
                                res.code = "5.00";
                                res.end(err.message);
                            });
                        }
                            ...
                            if (req.headers['Observe'] === 0) {
                                ...
                                var subscription = thing_1.subscribeEvent(segments[3], function (data) {
                                    ...
                                    try {
                                        ...
                                        console.warn("CoapServer on port " + _this.getPort() + " cannot process data for Event '" + segments[3] + ": " + err.message + "'");
                                        res.code = "5.00";
                                        res.end("Invalid Event Data");
                                        return;
                                    }
                                    console.log("CoapServer on port " + _this.getPort() + " sends '" + segments[3] + "' notification to " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
                                    res.setOption("Content-Format", content.type);
                                    res.code = "2.05";
                                    res.write(content.body);
                                })
                                    .then(function () {
                                    console.log("CoapServer on port " + _this.getPort() + " completes '" + segments[3] + "' subscription");
                                    res.end();
                                })
                                    .catch(function () {
                                    console.log("CoapServer on port " + _this.getPort() + " failed '" + segments[3] + "' subscription");
                                    res.code = "5.00";
                                    res.end();
                                });
                                res.on('finish', function () {
                                    console.log("CoapServer on port " + _this.getPort() + " ends '" + segments[3] + "' observation from " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
                                    thing_1.unsubscribeEvent(segments[3]);
                                });
                            }
                            else if (req.headers['Observe'] > 0) {
                                console.log("CoapServer on port " + this.getPort() + " sends '" + segments[3] + "' response to " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
                                res.code = "5.01";
                                res.end("node-coap issue: no GET cancellation, send RST");
                            }
                            else {
                                console.log("CoapServer on port " + this.getPort() + " rejects '" + segments[3] + "' read from " + core_1.Helpers.toUriLiteral(req.rsinfo.address) + ":" + req.rsinfo.port);
                                res.code = "4.00";
