...
var ConsumedThing = (function (_super) {
    ...
    ConsumedThing.prototype.readProperty = function (propertyName, options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var _a = _this.getClientFor(tp.forms, "readproperty"), client = _a.client, form = _a.form;
            console.log("form: " + form);
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' reading " + form.href);
    ...
    ConsumedThing.prototype.writeProperty = function (propertyName, value, options) {
        ...
        return new Promise(function (resolve, reject) {
            var tp = _this.properties[propertyName];
            var _a = _this.getClientFor(tp.forms, "writeproperty"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' writing " + form.href + " with '" + value + "'");
                var content = content_serdes_1.default.valueToContent(value, tp.input, form.contentType);
                form = _this.handleUriVariables(form, options);
    ...
    ConsumedThing.prototype.invokeAction = function (actionName, parameter, options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var _a = _this.getClientFor(ta.forms, "invokeaction"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' invoking " + form.href + (parameter !== undefined ? " with '" + parameter + "'" : ""));
                var input = void 0;
                if (parameter !== undefined) {
                    input = content_serdes_1.default.valueToContent(parameter, ta, form.contentType);
                }
                form = _this.handleUriVariables(form, options);
    ...
    ConsumedThing.prototype.observeProperty = function (name, listener, options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var _a = _this.getClientFor(tp.forms, "observeproperty"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' observing to " + form.href);
                form = _this.handleUriVariables(form, options);
    ...
    ConsumedThing.prototype.unobserveProperty = function (name) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var tp = _this.properties[name];
            var _a = _this.getClientFor(tp.forms, "unobserveproperty"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' unobserveing to " + form.href);
    ...
    ConsumedThing.prototype.subscribeEvent = function (name, listener, options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var _a = _this.getClientFor(te.forms, "subscribeevent"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' subscribing to " + form.href);
                form = _this.handleUriVariables(form, options);
                return client.subscribeResource(form, function (content) {
                    if (!content.type)
                        content.type = form.contentType;
    ...
    ConsumedThing.prototype.unsubscribeEvent = function (name) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var te = _this.events[name];
            var _a = _this.getClientFor(te.forms, "unsubscribeevent"), client = _a.client, form = _a.form;
            if (!client) {
                reject(new Error("ConsumedThing '" + _this.title + "' did not get suitable client for " + form.href));
            }
            else {
                console.log("ConsumedThing '" + _this.title + "' unsubscribing to " + form.href);
                client.unlinkResource(form);
                resolve();
            }
        });
